...
- Algorithm challenges have no automated grading yet: students Run their code (Docker-backed `code-execution.service`), but with the manual "mark complete" removed they currently have **no completion path** until the judge is extended to them. Main-course lessons with a `<slug>-tests.json` are judge-graded instead (see Lesson content)
...
- Gamification is derived client-side from `UserLessonProgress` counts — no separate gamification tables. **Badges**: "First Steps" (1 lesson) and Bronze/Silver/Gold tiers (10/20/30 lessons per course); new earnings trigger a 5s toast. **XP & levels**: XP is a _derived_ quantity, never stored nor accepted from a client — the server recomputes `earnedXp`/`totalXp` in `getCourseProgress` from the sortOrder of judge-completed lessons (`xpForLesson(sortOrder) = 10 + sortOrder`), so a tampered client can only fool its own display (completion rows are server-authoritative). Quadratic level curve `levelFromXp(xp) = floor(sqrt(xp/100)) + 1` with cosmos-themed titles (`level.title.1..10` i18n keys); `useGamification` aggregates XP across courses. Shown in Profile (level banner + progress bar), Topbar (level chip), and the lesson-completion toast (`+N XP`). No leaderboard
