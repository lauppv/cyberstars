...
- Lesson completion is driven **only** by passing the judge and is **server-authoritative**: `tests.controller.runTests` calls `progressService.markComplete` when the judge returns `passed` for a logged-in user. There is no client-writable completion endpoint (`POST/DELETE /api/progress/:courseKey/:lessonSlug/complete` were removed), so completion cannot be spoofed. The client reflects it optimistically then `refresh()`es. There is no manual "mark complete" button anywhere — Python and Java are judge-completable; courses without tests (C/Kotlin/Linux, algorithm challenges) are not completable yet
