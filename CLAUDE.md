...
- Linux lessons: each slug has 2 files — `<slug>.md` (no H1, space-station theme) and `<slug>-setup.json` (sandbox filesystem). 55 lessons. Shared types in `shared/terminal.ts`
...
- Lesson completion is driven **only** by passing the judge and is **server-authoritative**: `tests.controller.runTests` calls `progressService.markComplete` when the judge returns `passed` for a logged-in user. There is no client-writable completion endpoint (`POST/DELETE /api/progress/:courseKey/:lessonSlug/complete` were removed), so completion cannot be spoofed. The client reflects it optimistically then `refresh()`es. There is no manual "mark complete" button anywhere — Python, Java, and C main-course lessons plus the graded algorithm challenges (all algo-python/algo-java, core algo-c) are judge-completable; courses/challenges without tests (Kotlin/Linux, the 12 algo-c concurrency challenges) are not completable yet
