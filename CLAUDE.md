...
- Algorithm challenges have no automated grading yet: students Run their code (Docker-backed `code-execution.service`), but with the manual "mark complete" removed they currently have **no completion path** until the judge is extended to them — so XP badges are deliberately **not shown** on algorithm pages (AlgorithmListPage course total, per-challenge rewards, LessonPage header via `isAlgo`) to avoid promising unearnable XP. Main-course lessons with a `<slug>-tests.json` are judge-graded instead (see Lesson content)
...
- Lesson completion is driven **only** by passing the judge and is **server-authoritative**: `tests.controller.runTests` calls `progressService.markComplete` when the judge returns `passed` for a logged-in user. There is no client-writable completion endpoint (`POST/DELETE /api/progress/:courseKey/:lessonSlug/complete` were removed), so completion cannot be spoofed. The client reflects it optimistically then `refresh()`es. There is no manual "mark complete" button anywhere — Python, Java, and C are judge-completable; courses without tests (Kotlin/Linux, algorithm challenges) are not completable yet
