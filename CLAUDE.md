...
npm run build             # Production build: client to /dist + server to /dist-server (transpile-only via esbuild); runs gen:content first via prebuild
npm run build:server      # Transpile server/ + shared/ to dist-server/ (esbuild, no type-check) — see scripts/build-server.ts
...
- Prod runs **compiled JS** (`node dist-server/server/server.js`), not `tsx` — lower RAM/CPU on the 1GB box (PERF-AUDIT P6). `npm run build` produces `dist-server/` (transpile-only, mirrors tsx semantics). One-time pm2 switch: `pm2 delete cyberstars && pm2 start dist-server/server/server.js --name cyberstars` (was previously started on `tsx server/server.ts` / `npm start`)
