...
- Algorithm challenges have no automated grading yet: students Run their code (Docker-backed `code-execution.service`) and decide when to click "Mark Complete" themselves. Main-course lessons with a `<slug>-tests.json` are judge-graded instead (see Lesson content)
...
- Lesson completion is user-driven and toggleable: `POST /api/progress/:courseKey/:lessonSlug/complete` marks a lesson complete, `DELETE` on the same path un-marks it. The button updates optimistically and the client debounces the network call by 5s so rapid toggles coalesce (a keepalive `fetch` on `beforeunload` persists a pending toggle on tab close). Exception: lessons with judge tests hide the manual button — passing all tests marks them complete (see Lesson content)
