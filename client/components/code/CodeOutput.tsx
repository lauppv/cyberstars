...
export function CodeOutput({ output, height = "200px", fillHeight, isRunning, onInput }: CodeOutputProps) {
  ...
  return (
    <div className={`rounded-[var(--radius-sm)] border border-[var(--accent)]/30 overflow-hidden bg-[rgba(10,14,20,0.3)] backdrop-blur-[12px]${fillHeight ? " flex-1 min-h-0 flex flex-col" : ""}`}>
      <div className="px-3 py-1.5 text-[11px] uppercase tracking-[1px] text-[var(--text3)] font-semibold border-b border-[var(--accent)]/20 bg-[rgba(22,22,29,0.2)] flex items-center gap-2">
        <span>★ Output</span>
        {isRunning && <span className="inline-block w-1.5 h-1.5 rounded-full bg-[var(--success)] animate-pulse" />}
      </div>
      <div
        ref={containerRef}
        className={`p-4 font-mono text-[13px] overflow-auto whitespace-pre-wrap${fillHeight ? " flex-1 min-h-0" : ""}`}
        style={fillHeight ? { fontFamily: "var(--mono)" } : { height, minHeight: height, maxHeight: height, fontFamily: "var(--mono)" }}
          ...
          <>
            ...
            {isRunning && onInput && (
              <input
                ...
                style={{ fontFamily: "var(--mono)", width: `${Math.max(10, 30 - lastLine.length)}ch` }}
                autoFocus
