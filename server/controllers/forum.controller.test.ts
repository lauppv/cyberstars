...
describe("getThreads", () => {
  it("returns 404 for missing category", async () => {
    ...
    const next = vi.fn();
    await getThreads(mockReq({ params: { categorySlug: "nope" } as Record<string, string> }), mockRes(), next);
    expect(next).toHaveBeenCalledWith(expect.any(AppError));
    expect((next.mock.calls[0][0] as AppError).statusCode).toBe(404);
  });

  it("returns threads for a valid category", async () => {
    mockPrisma.forumCategory.findUnique.mockResolvedValue({ id: 1, slug: "general", name: "General", description: "", icon: "", color: "" });
    mockPrisma.forumThread.findMany.mockResolvedValue([
      {
        id: 10, title: "Hello", pinned: false, locked: false, solved: false, views: 3,
        authorId: 1, author: { name: "Alice", role: "USER" },
    ...
    const res = mockRes();
    await getThreads(mockReq({ params: { categorySlug: "general" } as Record<string, string> }), res, vi.fn());
...
describe("getThread", () => {
  it("returns 400 for invalid thread ID", async () => {
    const next = vi.fn();
    await getThread(mockReq({ params: { threadId: "abc" } as Record<string, string> }), mockRes(), next);
  ...
  it("returns 404 for missing thread", async () => {
    ...
    const next = vi.fn();
    await getThread(mockReq({ params: { threadId: "999" } as Record<string, string> }), mockRes(), next);
    expect((next.mock.calls[0][0] as AppError).statusCode).toBe(404);
  });

  it("returns thread detail and increments views", async () => {
    mockPrisma.forumThread.findUnique.mockResolvedValue({
      id: 1, title: "T", pinned: false, locked: false, solved: false, views: 5,
      authorId: 1, author: { name: "A", role: "USER" },
      category: { slug: "general", name: "General" },
      createdAt: new Date(), posts: [],
    });
...
describe("createThread", () => {
  ...
  it("returns 404 for unknown category", async () => {
    ...
    const next = vi.fn();
    await createThread(
      mockReq({ user: { id: 1 } as Request["user"], body: { categorySlug: "nope", title: "T", content: "C" } }),
      mockRes(), next,
    );
    expect((next.mock.calls[0][0] as AppError).statusCode).toBe(404);
  });
  ...
  it("returns 403 for USER posting in announcements", async () => {
    ...
    const next = vi.fn();
    await createThread(
      mockReq({ user: { id: 1 } as Request["user"], body: { categorySlug: "announcements", title: "T", content: "C" } }),
      mockRes(), next,
    );
  ...
  it("creates thread successfully", async () => {
    ...
    const res = mockRes();
    await createThread(
      mockReq({ user: { id: 1 } as Request["user"], body: { categorySlug: "general", title: "Hello", content: "World" } }),
      res, vi.fn(),
    );
...
describe("createPost", () => {
  it("returns 404 for missing thread", async () => {
    ...
    const next = vi.fn();
    await createPost(
      mockReq({ user: { id: 1 } as Request["user"], params: { threadId: "1" } as Record<string, string>, body: { content: "hi" } }),
      mockRes(), next,
    );
    expect((next.mock.calls[0][0] as AppError).statusCode).toBe(404);
  });
  ...
  it("returns 403 for locked thread", async () => {
    mockPrisma.forumThread.findUnique.mockResolvedValue({ id: 1, locked: true, category: { slug: "general" } });
    const next = vi.fn();
    await createPost(
      mockReq({ user: { id: 1 } as Request["user"], params: { threadId: "1" } as Record<string, string>, body: { content: "hi" } }),
      mockRes(), next,
  ...
  it("creates post successfully", async () => {
    mockPrisma.forumThread.findUnique.mockResolvedValue({ id: 1, locked: false, category: { slug: "general" } });
...
describe("toggleReaction", () => {
  it("removes existing reaction", async () => {
    ...
    await toggleReaction(
      mockReq({ user: { id: 1 } as Request["user"], params: { postId: "1" } as Record<string, string>, body: { emoji: "👍" } }),
      res, vi.fn(),
    );
...
describe("markSolution", () => {
  it("returns 403 if not thread author", async () => {
    ...
    const next = vi.fn();
    await markSolution(
      mockReq({ user: { id: 1 } as Request["user"], params: { postId: "1" } as Record<string, string> }),
      mockRes(), next,
    );
    expect((next.mock.calls[0][0] as AppError).statusCode).toBe(403);
  });
  ...
  it("marks post as solution", async () => {
    mockPrisma.forumPost.findUnique.mockResolvedValue({ id: 1, threadId: 10, thread: { authorId: 1 } });
...
describe("updatePost", () => {
  it("returns 403 when USER tries to edit another user's post", async () => {
    mockPrisma.forumPost.findUnique.mockResolvedValue({ id: 1, authorId: 99, author: { role: "USER" } });
    mockUserRepo.getRole.mockResolvedValue("USER");
    const next = vi.fn();
    await updatePost(
      mockReq({ user: { id: 1 } as Request["user"], params: { postId: "1" } as Record<string, string>, body: { content: "edited" } }),
      mockRes(), next,
    );
    expect((next.mock.calls[0][0] as AppError).statusCode).toBe(403);
  });
  ...
  it("allows ADMIN to edit any post", async () => {
    mockPrisma.forumPost.findUnique.mockResolvedValue({ id: 1, authorId: 99, author: { role: "MODERATOR" } });
  ...
  it("prevents MODERATOR from editing ADMIN's post", async () => {
    mockPrisma.forumPost.findUnique.mockResolvedValue({ id: 1, authorId: 99, author: { role: "ADMIN" } });
    mockUserRepo.getRole.mockResolvedValue("MODERATOR");
    const next = vi.fn();
    await updatePost(
      mockReq({ user: { id: 1 } as Request["user"], params: { postId: "1" } as Record<string, string>, body: { content: "edited" } }),
...
describe("deletePost", () => {
  it("soft-deletes post with actor name", async () => {
    mockPrisma.forumPost.findUnique.mockResolvedValue({ id: 1, authorId: 1, author: { role: "USER" } });
    mockUserRepo.getRole.mockResolvedValue("USER");
    ...
    expect(mockPrisma.forumPost.update).toHaveBeenCalledWith(expect.objectContaining({
...
describe("deleteThread", () => {
  it("returns 404 for missing thread", async () => {
    ...
    const next = vi.fn();
    await deleteThread(
      mockReq({ user: { id: 1 } as Request["user"], params: { threadId: "1" } as Record<string, string> }),
      mockRes(), next,
    );
    expect((next.mock.calls[0][0] as AppError).statusCode).toBe(404);
  });
  ...
  it("deletes thread by owner", async () => {
    mockPrisma.forumThread.findUnique.mockResolvedValue({ id: 1, authorId: 1, author: { role: "USER" } });
    mockUserRepo.getRole.mockResolvedValue("USER");
...
describe("updateUserRole", () => {
  it("returns 403 for non-admin", async () => {
    ...
    await updateUserRole(
      mockReq({ user: { id: 1 } as Request["user"], params: { userId: "2" } as Record<string, string>, body: { role: "MODERATOR" } }),
      mockRes(), next,
    );
    expect((next.mock.calls[0][0] as AppError).statusCode).toBe(403);
  });
  ...
  it("returns 400 when admin tries to change own role", async () => {
    ...
    const next = vi.fn();
    await updateUserRole(
      mockReq({ user: { id: 1 } as Request["user"], params: { userId: "1" } as Record<string, string>, body: { role: "USER" } }),
      mockRes(), next,
    );
    expect((next.mock.calls[0][0] as AppError).statusCode).toBe(400);
  });
  ...
  it("returns 400 for invalid role", async () => {
    ...
    const next = vi.fn();
    await updateUserRole(
      mockReq({ user: { id: 1 } as Request["user"], params: { userId: "2" } as Record<string, string>, body: { role: "SUPERADMIN" } }),
      mockRes(), next,
    );
    expect((next.mock.calls[0][0] as AppError).statusCode).toBe(400);
  });
