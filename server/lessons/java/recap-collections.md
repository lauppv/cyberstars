...
Store the purchases in a `String[] purchases` array. Count each weapon in a `HashMap`. Find the weapon with the most purchases (`"Most bought: X"`). Collect weapons bought more than once into an `ArrayList`, sort it alphabetically with `Collections.sort`, and print everything (`"Popular (sorted): [...]"`)
