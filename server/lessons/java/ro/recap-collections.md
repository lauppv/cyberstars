...
Stochează achizițiile într-un array `String[] achizitii`. Numără fiecare armă într-un `HashMap`. Găsește arma cu cele mai multe achiziții (`"Cel mai cumparat: X"`). Colectează armele cumpărate de mai mult de o dată într-un `ArrayList`, sortează-l alfabetic cu `Collections.sort` și afișează totul (`"Populare (sortat): [...]"`)
