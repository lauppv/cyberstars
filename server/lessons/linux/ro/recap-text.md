...
Folosește `cut` ca să extragi doar coloana cu nume (câmpul 1, delimitat prin virgulă), trimite prin pipe la `sort`, apoi la `uniq -c` ca să numere aparițiile și redirectează rezultatul în `raport.txt` cu `>`.
