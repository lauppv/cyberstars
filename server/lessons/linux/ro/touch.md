...
Comandantul stației are nevoie de trei fișiere noi pentru briefing-ul de astăzi. Directorul tău home are deja pregătit un folder `reports`.

1. Creează un fișier gol numit `mission.txt`.
2. Creează un fișier gol numit `crew.txt`.
3. Creează un fișier gol numit `summary.txt` **în interiorul** folderului `reports`.

**Rezultat așteptat**

Rulând `ls` în directorul tău home apar `mission.txt` și `crew.txt`, iar rulând `ls reports` apare `summary.txt`.
