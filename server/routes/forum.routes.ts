...
const forumWriteLimiter = rateLimit({ windowMs: 60_000, limit: 10 });
...
router.post("/threads", authenticateToken, forumWriteLimiter, validateBody(createThreadSchema), forumController.createThread);
router.post("/threads/:threadId/posts", authenticateToken, forumWriteLimiter, validateBody(createPostSchema), forumController.createPost);
router.delete("/threads/:threadId", authenticateToken, forumController.deleteThread);

router.post("/posts/:postId/reactions", authenticateToken, forumWriteLimiter, validateBody(toggleReactionSchema), forumController.toggleReaction);
router.post("/posts/:postId/solution", authenticateToken, forumController.markSolution);
router.put("/posts/:postId", authenticateToken, forumWriteLimiter, validateBody(updatePostSchema), forumController.updatePost);
