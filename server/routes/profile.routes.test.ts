...
describe("PATCH /api/profile", () => {
  ...
  it("updates bio", async () => {
    ...
    expect(mockUserRepo.updateProfile).toHaveBeenCalledWith(1, expect.objectContaining({ bio: "Hello world" }));
  });
  ...
  it("clears bio when empty string", async () => {
    ...
    expect(res.status).toBe(200);
    expect(mockUserRepo.updateProfile).toHaveBeenCalledWith(1, expect.objectContaining({ bio: null }));
  });
  ...
  it("sets status with 24h expiry", async () => {
    ...
    expect(call[1].statusExpiresAt.getTime()).toBeGreaterThanOrEqual(before + 24 * 60 * 60 * 1000 - 1000);
  });
...
describe("POST /api/profile/avatar", () => {
  ...
  it("uploads avatar successfully", async () => {
    ...
    expect(mockUserRepo.updateProfile).toHaveBeenCalledWith(1, expect.objectContaining({ avatarUrl: expect.stringContaining("/uploads/avatars/") }));
  });
